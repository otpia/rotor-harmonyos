...
Rotor 身份验证器是一款面向 HarmonyOS 的两步验证工具，支持离线生成验证码、扫码迁移、服务图标、加密备份与可选云同步。本文档提供上架文案与宣传图素材。
...
```text
Rotor 身份验证器是一款面向 HarmonyOS 的两步验证工具。将网站或 App 提供的验证器账号添加到 Rotor，即可离线生成动态验证码，登录时点按复制使用。

- 离线取码：支持基于时间的 TOTP 和基于计数器的 HOTP。验证码大字号显示，点按即可复制；TOTP 圆环提示剩余时间，临近过期时变色提醒。
- 扫码迁移：扫描二维码、识别相册二维码，或手动输入设置密钥。支持 Google 身份验证器导出的迁移二维码，一次导入其中的多个账号，重复账号自动跳过。
- 服务图标：按账号名称自动匹配图标，也可搜索内置图标，或从图库选择图片作为自定义图标。
- 加密备份：选择账号并设置密码，导出 AES-256 加密的 .rotorbak 文件。换机后导入文件、输入密码即可恢复，重复账号自动跳过。
- 可选云同步：通过华为云空间，在登录同一华为账号的 HarmonyOS 设备间同步账号与密钥。需同时开启应用内与系统云空间中的 Rotor 同步开关。图库图标仅保存在本机。
- 账号整理：按名称和描述搜索，长按拖动排序，左滑编辑与删除，支持批量删除和导出。
- 原生界面：底部悬浮栏集中提供搜索、添加与更多操作，深浅色模式跟随系统。

适用于支持 TOTP 或 HOTP 标准的两步验证服务（2FA）。可设置 15、30、60 秒周期，6 位或 8 位验证码，以及 SHA1、SHA256、SHA512 算法；参数需与对应服务一致。
```

## 2.0.0 版本特性

```text
- 新增可选云同步：通过华为云空间，在同一华为账号的 HarmonyOS 设备间同步账号与密钥。
- 新增服务图标库：按账号名称自动识别图标，支持搜索选择与图库导入。
- 更新 HarmonyOS 原生界面：搜索、添加与更多操作集中到底部悬浮栏，支持沉浸光感与系统深浅色模式。
- 优化账号列表的拖动排序、宽屏布局与按钮点击区域。
```

## 文案依据

检索日期：2026 年 9 月 25 日。文案以当前代码和商店字段为准，竞品资料用于比较用户关注的功能。

| 官方资料 | 用户关注点 | Rotor 的对应表述 |
| --- | --- | --- |
| [Google 身份验证器](https://play.google.com/store/apps/details?hl=zh&id=com.google.android.apps.authenticator2) | 离线生成、扫码添加、二维码迁移。 | 首句说明使用场景，迁移注明支持 Google 身份验证器导出的二维码。 |
| [2FAS Auth](https://2fas.com/auth/) | 离线使用、备份与恢复。 | 区分离线取码、密码加密文件备份与可选云同步。 |
| [Ente Auth](https://ente.com/auth/) | 多设备访问、图标与账号查找。 | 明确同步范围为同一华为账号的 HarmonyOS 设备，说明图标与搜索能力。 |
| [Aegis](https://getaegis.app/) | 加密备份、图标与账号管理。 | 用具体操作描述备份恢复和账号整理。 |
| [华为应用提交指南](https://developer.huawei.com/consumer/cn/app/submit/) | 应用资料、素材和发布包。 | 介绍、版本特性、宣传图与发布包保持一致；草稿保存与提交审核分别操作。 |

简介突出「两步验证、离线、迁移、云同步」，名称保留「Rotor 身份验证器」。加密强度仅用于描述文件备份，云同步说明实际通道和设备范围。
...
扫码使用 Scan Kit 默认界面，备份导入导出使用系统文件选择器，从图库选择图标使用系统图库选择器。应用未声明 `CAMERA`、`INTERNET`、`READ_MEDIA`、`WRITE_MEDIA` 权限。
...
- 自定义图标：从图库选择的图标缩小后保存在应用沙箱的 `files/issuer_icons` 目录，只保存在本机。开启云同步后，账号的图标选择随账号同步，其他设备上没有该图片时按名称自动识别。
