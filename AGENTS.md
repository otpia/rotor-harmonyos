...
- 账号 `iconKey` 记录手动选择的图标：空串按名称自动识别，`lib:<资源键>` 为内置图标，`img:<文件名>` 为从图库导入的图标（存于 `files/issuer_icons`，只在本机，其他设备回退到自动识别）；编辑页点头像或「更换图标」打开 `IconPicker` 半模态
- 不声明 `ohos.permission.INTERNET`，上架时备案信息选「单机 APP」
...
- `services/IconCache.ets`：图库图标的导入、缩小与本地存储
- `components/IconPicker.ets`：图标选择半模态（搜索内置图标、自动识别、图库）
